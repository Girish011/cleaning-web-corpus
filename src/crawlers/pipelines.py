...
class CleaningImagesPipeline(ImagesPipeline):
    ...
    def file_path(self, request, response=None, info=None, *, item=None):
        ...
        if item and 'url' in item:
            page_id = hashlib.md5(item['url'].encode()).hexdigest()[:8]
        else:
            page_id = 'unknown'
        
        # Generate image hash from URL
        image_hash = hashlib.sha1(image_url.encode()).hexdigest()[:16]
    ...
    def item_completed(self, results, item, info):
        ...
        for success, result in results:
            if success:
                ...
                try:
                    image_path = pathlib.Path(info.spider.settings.get('IMAGES_STORE', 'data/images')) / image_info['path']
