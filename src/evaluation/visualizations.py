...
class AblationVisualizer:
    ...
    def plot_documents_passed_failed(self) -> pathlib.Path:
        ...
        bars2 = ax.bar(x, failed, width, bottom=passed, label='Failed', color='#F44336', alpha=0.8, edgecolor='black', linewidth=1.5)
        ...
        # Add baseline line
        if baseline_count > 0:
            ax.axhline(y=baseline_count, color='blue', linestyle='--', linewidth=2, alpha=0.5, label=f'Baseline: {baseline_count}')
            ax.legend(fontsize=11)
